...
public class Toolkit {
	...
	public static boolean inArena(World world) {
		...
		if (Game.getInstance().getResources().getConfig().contains("Arenas")) {
			...
			Toolkit.printToConsole("[SoupPvP] &cThere was no spawn found, please set it using /Kp addspawn.");
		}
		return false;
 	...
 	public static void runCommands(Player p, List<String> commands, String replaceFrom, String replaceTo) {
		...
		for (String commandString : commands) {
			...
			if (commandPhrase.length == 1) {
				Toolkit.printToConsole("[SoupPvP] &cIncorrect command format. Please see: &fhttps://bit.ly/kp-command-format");
				return;
			}
			...
			if (sender.equals("console")) {
				...
				Toolkit.printToConsole("[SoupPvP] Incorrect command format. Please see: &fhttps://bit.ly/kp-command-format");
				return;
